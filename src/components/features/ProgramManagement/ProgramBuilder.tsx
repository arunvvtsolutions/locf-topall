...
export const ProgramBuilder = ({ organizationId, onComplete, yearsSelectOption }: ProgramBuilderProps) => {
  ...
  const generateYearStructures = (duration: number, totalSemesters: number) => {
    console.log(`Generating structures for ${duration} years and ${totalSemesters} semesters`);
    const years: YearStructure[] = [];
    const semestersPerYear = Math.ceil(totalSemesters / duration);
    console.log(`Semesters per year: ${semestersPerYear}`);
    let semesterCounter = 1;

    for (let year = 1; year <= duration; year++) {
      const semesters: SemesterStructure[] = [];

      for (let semInYear = 1; semInYear <= semestersPerYear && semesterCounter <= totalSemesters; semInYear++) {
    ...
    console.log('Generated year structures:', years);
    return years;
  };

  const handleProgramDataChange = (data: Partial<ProgramData>) => {
    console.log('Program data changed:', data);
    setProgramData(prev => ({
      ...prev,
      ...data
    }));
  ...
  const handleNext = () => {
    if (currentStep === 0) {
      const structures = generateYearStructures(programData.durationYears, programData.totalSemesters);
 ...
 const handleCreateProgram = async () => {
  ...
  const canProceed = () => {
    if (currentStep === 0) {
      return programData.name && programData.code && programData.durationYears > 0 && programData.totalSemesters > 0;
    }
    return true;
  };
  ...
  useEffect(() => {
    if (programData.durationYears > 0 && programData.totalSemesters > 0) {
      console.log('Regenerating year structures due to duration/semester change');
      const structures = generateYearStructures(programData.durationYears, programData.totalSemesters);
      console.log('Generated year structures:', structures);
      setYearStructures(structures);
    } else {
            ...
            <CardContent>
              <div className="space-y-4">
                <div className="bg-green-50 p-4 rounded-lg text-green-800 text-center">
                  <p>
                    <span className="font-semibold">{programData.name}</span> ({programData.durationYears} years, {programData.totalSemesters} semesters)
                  </p>
                  <p className="text-sm mt-1 mb-1">
                    All academic years and semesters are set up!
                  </p>
                </div>
